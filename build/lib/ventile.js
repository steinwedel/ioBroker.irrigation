...
class ValveController {
  ...
  async init() {
    ...
    await this.adapter.delObjectAsync(`${this.id}.flowSensorId`).catch(() => void 0);
   ...
   */
  async setHomematicOnTime(value) {
    try {
  ...
  async start(durationSecs) {
    ...
    this.clearTickTimer();
    const effectiveDurationSecs = typeof durationSecs === "number" && durationSecs > 0 ? durationSecs : this.config.duration;
